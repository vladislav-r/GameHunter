	...
	return (
		<div className='games'>
			<div className='container'>
				...
				<a className='games-inner' style={GamesStyle} path='/game' component={Game}>
					{games.map(game => (
						<div className='game' style={GameStyle} key={game.id}>
							...
							<div>
								<h3>{game.title}</h3>
								<p>
									{game.originalPrice} <span>{game.priceCurrency}</span>
								</p>
								<p>
									Со скидкой: {game.discountedPrice} <span>{game.priceCurrency}</span>
								</p>
